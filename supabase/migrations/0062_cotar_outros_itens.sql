-- Cotação (fornecedor): além dos itens que ele já fornece, mostrar os OUTROS
-- itens pedidos na cotação (que ele ainda não fornece). Se ele trabalha com
-- algum, pode se incluir e cotar — o vínculo fornecedor_produto é criado.
...
-- Salvar: aceita preço para QUALQUER item da cotação (inclusive os "outros").
-- Ao gravar um preço, garante o vínculo fornecedor_produto (o fornecedor passa
-- a fornecer aquele item).
...
begin
  ...
  -- Válidos = todos os itens da cotação (não só os já vinculados a ele).
  ...
  loop
    ...
    if v_pid = any(v_valid) then
      -- Garante o vínculo (fornecedor passa a fornecer o item).
