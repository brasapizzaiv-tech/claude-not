-- Link público por fornecedor para preencher os preços da cotação.
alter table public.cotacao_fornecedores
  add column if not exists token text unique;

-- Dados para o fornecedor preencher (valida token). Retorna a cotação, o nome
-- do fornecedor e os produtos da cotação que ESTE fornecedor fornece
-- (com a quantidade pedida e o preço já informado, se houver).
...
create or replace function public.cotar_fornecedor_salvar(p_token text, p_precos jsonb)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  -- Produtos válidos: itens da cotação que este fornecedor fornece.
  ...
  loop
    if (v_item->>'produto_id')::uuid = any(v_valid) then
      insert into cotacao_precos
        (cotacao_id, fornecedor_id, produto_id, preco_unit, disponivel)
      values (
  ...
  update cotacao_fornecedores set status = 'respondido' where token = p_token;
