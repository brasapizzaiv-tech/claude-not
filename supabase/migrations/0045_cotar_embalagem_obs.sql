-- Cotação (fornecedor): embalagem/unidade de venda + observação por item,
-- e suporte a rascunho (auto-save) que não marca a cotação como "respondido".

alter table public.cotacao_precos
  add column if not exists embalagem  text,
  add column if not exists observacao text;

-- Dados para o fornecedor preencher (agora com embalagem/observação por item).
...
-- Salvar: agora grava embalagem/observação por item. Se p_dados->>'rascunho'
-- for verdadeiro (auto-save), NÃO marca a cotação como "respondido".
