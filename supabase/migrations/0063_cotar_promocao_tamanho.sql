-- Cotação (fornecedor): promoção/oferta (texto + foto) por fornecedor, e
-- tamanho da embalagem por item (ex.: "12 un", "5 kg") — separa preços que só
-- diferem pelo tamanho do pacote.

alter table public.cotacao_fornecedores
  add column if not exists promocao_texto text,
  add column if not exists promocao_foto  text;

alter table public.cotacao_precos
  add column if not exists tamanho_embalagem text;
