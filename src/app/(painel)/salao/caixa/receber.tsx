...
export function ReceberComandas({
  ...
  const subtotalBruto = Math.round(linhasCarrinho.reduce((s, l) => s + l.valor, 0) * 100) / 100;
  ...
  function confirmar() {
    ...
    // recibo: total por comanda no carrinho
    ...
    setMsg(null);
    start(async () => {
      const r = await pagarSelecao(payload, pagamentos);
      if (r.ok) {
        setMsg(
  ...
  return (
    <div>
      {/* Topo: comandas selecionadas + busca para adicionar mais */}
      <div className="mb-3 rounded-2xl border border-zinc-200 p-3 dark:border-zinc-800">
        <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-zinc-400">Comandas</p>
