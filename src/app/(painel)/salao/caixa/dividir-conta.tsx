    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-lg rounded-cartao bg-painel-cartao p-5">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-texto-fraco hover:text-texto-suave">✕</button>
        </div>
        ...
        {linhas.length === 0 ? (
          <p className="rounded-cartao bg-painel-cartao p-6 text-center text-sm text-emerald-600">
            ✓ Tudo pago!
          </p>
        ) : (
          <>
            <div className="max-h-60 overflow-y-auto rounded-cartao bg-painel-cartao">
              {linhas.map((l) => (
                <div
                  key={l.key}
                  className={`flex items-center gap-2 border-b border-borda px-3 py-2 text-sm last:border-0 ${
                    sel.has(l.key) ? "bg-orange-50 dark:bg-orange-950/20" : ""
                  }`}
                >
                  <input type="checkbox" checked={sel.has(l.key)} onChange={() => toggle(l)} />
                  <span className="flex-1 text-texto">{l.nome}</span>
                  {sel.has(l.key) ? (
                    <input
                      inputMode="decimal"
                      value={pagarDe[l.key] ?? ""}
                      onChange={(e) => setPagarDe((p) => ({ ...p, [l.key]: e.target.value }))}
                      className="w-20 rounded border border-borda-forte bg-painel-cartao px-2 py-1 text-right text-sm"
                    />
                  ) : (
                    <span className="font-medium text-texto-suave">{brl(l.restante)}</span>
                  )}
                  <button
                    onClick={() => metade(l)}
            ...
            <div className="mt-3 flex items-baseline justify-between">
              <span className="text-xs text-texto-suave">falta {brl(restanteTotal)}</span>
              <span className="text-2xl font-black text-texto">{brl(somaSel)}</span>
            </div>
            ...
            {somaSel > 0.005 && (
              <div className="mt-2 space-y-2">
                ...
                <button
                  ...
                  className="w-full rounded-controle bg-texto py-2.5 text-sm font-semibold text-fundo hover:opacity-90 disabled:opacity-50"
                >
        ...
        <button onClick={onClose} className="mt-3 w-full text-center text-xs text-texto-fraco hover:text-texto-suave">
          Fechar
        </button>
      </div>
    </div>
