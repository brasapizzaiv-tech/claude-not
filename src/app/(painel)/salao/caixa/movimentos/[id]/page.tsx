...
export default async function RecebimentoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl p-6">
      <Link href="/salao/caixa/movimentos" className="text-sm text-zinc-500 hover:text-orange-600">← Movimentações</Link>
      <h1 className="mt-2 text-2xl font-bold text-zinc-900 dark:text-zinc-50">Recebimento</h1>
      ...
      {/* Pagamento */}
      <div className={card}>
        <p className="mb-2 text-sm font-semibold text-zinc-900 dark:text-zinc-100">Como foi pago</p>
        <ul className="divide-y divide-zinc-100 dark:divide-zinc-800">
          {pagamentos.map((p) => (
            <li key={p.id} className="flex flex-wrap items-baseline justify-between gap-2 py-2 text-sm">
              <span className="text-zinc-800 dark:text-zinc-100">
                {p.forma_pagamento ?? "—"}
                {p.bandeira && <span className="ml-2 rounded bg-zinc-100 px-1.5 py-0.5 text-[11px] text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">{p.bandeira}</span>}
                {p.tef_nsu && (
                  <span className="ml-2 text-xs text-zinc-500">
                    pinpad · NSU {p.tef_nsu}{p.tef_autorizacao ? ` · aut ${p.tef_autorizacao}` : ""}{p.tef_rede ? ` · ${p.tef_rede}` : ""}
                  </span>
                )}
                {p.observacao && <span className="ml-2 text-xs text-zinc-500">{p.observacao}</span>}
                <span className="ml-2 text-xs text-zinc-400">{hora(p.criado_em)}</span>
              </span>
      ...
      <div className="mt-4 space-y-3">
        {comandas.length === 0 && (
          <p className={`${card} text-sm text-zinc-500`}>Este lançamento não tem comanda ligada.</p>
        )}
        {comandas.map((c) => {
            ...
            <div key={c.id} className={card}>
              <div className="mb-2 flex flex-wrap items-baseline justify-between gap-2">
                <p className="font-semibold text-zinc-900 dark:text-zinc-50">
                  Comanda #{c.numero}
                  {c.mesa ? <span className="ml-2 text-sm font-normal text-zinc-500">{c.mesa}</span> : null}
              ...
              <ul className="divide-y divide-zinc-100 text-sm dark:divide-zinc-800">
                {buffet > 0 && (
                  <li className="flex justify-between py-1.5">
                    <span className="text-zinc-700 dark:text-zinc-200">Buffet{c.livre ? " (livre)" : ""}</span>
                    <span className="text-zinc-600 dark:text-zinc-300">{brl(buffet)}</span>
                  </li>
                )}
                {meus.map((i) => (
                  <li key={i.id} className="flex justify-between gap-2 py-1.5">
                    ...
                    <span className="text-zinc-600 dark:text-zinc-300">{brl(Number(i.qtd) * Number(i.preco_unit))}</span>
                  </li>
                ))}
                {meus.length === 0 && buffet === 0 && <li className="py-1.5 text-zinc-400">Sem itens.</li>}
              </ul>
              <div className="mt-2 flex justify-between border-t border-zinc-100 pt-2 text-sm dark:border-zinc-800">
                <span className="text-zinc-500">
                  Subtotal{servico > 0 ? ` + serviço ${brl(servico)}` : ""}
                  {c.fechada_em ? ` · fechada ${hora(c.fechada_em)}` : ""}
                </span>
                <b className="text-zinc-900 dark:text-zinc-50">{brl(buffet + somaItens + servico)}</b>
              </div>
            </div>
          );
        })}
