...
export function ItensClient({ categorias, itens }: { categorias: CatRow[]; itens: ItemRow[] }) {
  ...
  return (
    <div className="space-y-6">
      {msg && <div className="rounded-controle bg-green-50 px-3 py-2 text-sm text-green-700 dark:bg-green-950 dark:text-green-300">{msg}</div>}

      {/* Categorias */}
      <details className="rounded-cartao border border-borda p-4">
        <summary className="cursor-pointer font-semibold text-texto">Categorias ({categorias.length})</summary>
          ...
          {categorias.map((c) => {
            ...
            return (
              <div key={c.id} className="flex items-center gap-2">
                ...
                <button onClick={() => salvarCat(c)} disabled={!mudou || salvando} className="rounded-controle bg-orange-500 px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-40">Salvar</button>
              </div>
            );
          })}
          <div className="flex items-center gap-2">
            <input value={novaCat} onChange={(e) => setNovaCat(e.target.value)} placeholder="Nova categoria" className={input} />
            <button onClick={criarCat} disabled={!novaCat.trim() || salvando} className="rounded-controle bg-zinc-900 px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-40 dark:bg-white dark:text-zinc-900">＋</button>
          </div>
        </div>
      </details>

      {/* Novo item */}
      <div className="rounded-cartao border border-orange-200 bg-orange-50/40 p-4 dark:border-orange-900 dark:bg-orange-950/10">
        <p className="mb-2 font-semibold text-texto">＋ Novo item</p>
        <div className="grid gap-2 sm:grid-cols-[2fr_1.5fr_repeat(3,minmax(0,1fr))_auto]">
          ...
          <button onClick={criar} disabled={!novo.nome.trim() || salvando} className="rounded-controle bg-orange-500 px-4 py-1.5 text-sm font-semibold text-white disabled:opacity-40">Criar</button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-sm text-texto-suave">{lista.length} item(ns)</span>
      </div>
                ...
                return (
                  <tr key={i.id} className={`bg-painel-cartao ${!i.ativo ? "opacity-50" : ""}`}>
                    ...
                    <td className="px-3 py-1.5 text-right whitespace-nowrap">
                      <button onClick={() => salvar(i)} disabled={!alterado(i) || salvando} className="rounded-controle bg-orange-500 px-3 py-1 text-xs font-semibold text-white disabled:opacity-30">Salvar</button>
