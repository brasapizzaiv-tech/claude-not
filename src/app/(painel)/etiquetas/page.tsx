    ...
    <div className="w-full p-8">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-texto-suave">Gere etiquetas de manipulação e controle a validade dos insumos.</p>
        </div>
        <div className="flex shrink-0 flex-wrap justify-end gap-2">
          <Link href="/etiquetas/impressas" className="rounded-cartao border border-borda-forte px-4 py-2.5 text-sm font-semibold text-texto-suave hover:bg-superficie-suave dark:border-borda-forte">
            <Icone nome="subindo" tamanho={15} className="mr-1.5" /> Impressas
          </Link>
          <Link href="/etiquetas/itens" className="rounded-cartao border border-borda-forte px-4 py-2.5 text-sm font-semibold text-texto-suave hover:bg-superficie-suave dark:border-borda-forte">
            <Icone nome="lista" tamanho={15} className="mr-1.5" /> Itens e categorias
          </Link>
          <Link href="/impressao" className="rounded-cartao border border-borda-forte px-4 py-2.5 text-sm font-semibold text-texto-suave hover:bg-superficie-suave dark:border-borda-forte">
            <Icone nome="imprimir" tamanho={15} className="mr-1.5" /> Impressoras
          </Link>
          <Link href="/etiquetas/scanner" className="rounded-cartao bg-zinc-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-zinc-700 dark:bg-white dark:text-zinc-900">
      ...
      <div className="mb-6 space-y-3">
        <p className="text-xs font-semibold text-texto-fraco">Painel de vencimentos</p>
        <div className="grid gap-2 lg:grid-cols-[1fr_auto]">
          <PainelVencimentos contagem={contagem} base="/etiquetas" ativo={faixa} />
          <div className="grid grid-cols-2 gap-2 lg:w-64">
            <Link href={soVencidas ? "/etiquetas" : "/etiquetas?f=vencidas"} className={`rounded-cartao border-2 p-3 ${soVencidas ? "border-red-500 bg-red-50 dark:bg-red-950/30" : "border-borda"}`}>
              <div className="text-3xl font-black leading-none text-red-600">{contagem.vencidas}</div>
              <div className="mt-1 text-xs font-semibold text-texto-suave">Vencidas não baixadas</div>
            </Link>
            <Link href="/etiquetas?ver=historico" className="rounded-cartao border-2 border-borda p-3">
              <div className="text-3xl font-black leading-none text-texto-suave">{baixadas ?? 0}</div>
              <div className="mt-1 text-xs font-semibold text-texto-suave">Baixadas</div>
            </Link>
          </div>
        </div>
      ...
      {itens.length === 0 && (
        <div className="mb-4 rounded-cartao border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-200">
      ...
      <div className="mt-8 mb-3 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <Link href="/etiquetas" className={`rounded-controle px-3 py-1.5 text-sm font-medium ${!historico ? "bg-orange-500 text-white" : "border border-borda-forte text-texto-suave  "}`}>
            Ativas
          </Link>
          <Link href="/etiquetas?ver=historico" className={`rounded-controle px-3 py-1.5 text-sm font-medium ${historico ? "bg-orange-500 text-white" : "border border-borda-forte text-texto-suave  "}`}>
            Histórico (baixadas)
          </Link>
          {filtroAtivo && (
