...
export default async function CentralImpressaoPage() {
  ...
  const [{ data }, { data: prods }] = await Promise.all([
    supabase.from("impressoras").select("id, nome, ativo, impressora_windows, recebe_comandas, recebe_marmitas, recebe_nfce, recebe_pizzas, comanda_produtos, comanda_config, etiqueta_config").order("criado_em"),
