...
export type Perfil = { id: string; nome: string; ncm: string | null; cest: string | null; cfop: string; csosn: string; origem: string; unidade: string; pis_cst: string; cofins_cst: string; homologado: boolean; obs: string | null; ativo: boolean };
...
const VAZIO: Omit<Perfil, "id" | "ativo"> = { nome: "", ncm: "", cest: "", cfop: "5102", csosn: "102", origem: "0", unidade: "UN", pis_cst: "49", cofins_cst: "49", homologado: false, obs: "" };
              ...
              {perfis.map((p) => (
                <tr key={p.id} className={`bg-white dark:bg-zinc-950 ${!p.ativo ? "opacity-50" : ""}`}>
                  ...
                  <td className="px-3 py-2 text-right whitespace-nowrap">
                    <button onClick={() => setEditando({ id: p.id, nome: p.nome, ncm: p.ncm ?? "", cest: p.cest ?? "", cfop: p.cfop, csosn: p.csosn, origem: p.origem, unidade: p.unidade, pis_cst: p.pis_cst, cofins_cst: p.cofins_cst, homologado: p.homologado, obs: p.obs ?? "" })} className="mr-3 text-orange-600 hover:underline">Editar</button>
                    <button
