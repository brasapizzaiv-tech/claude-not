  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: temRecado ? "flex-start" : "center", padding: temRecado ? "28px 32px 0" : "0 32px" }}>
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 24 }}>
        <div style={{ fontSize: vh(temRecado ? 150 : 220), lineHeight: 1, fontWeight: 900, letterSpacing: "0.02em", color: "#fff", fontVariantNumeric: "tabular-nums" }}>
          {p.hora}<span style={{ color: "#C78340", visibility: apagado ? "hidden" : "visible" }}>:</span>{p.min}
        </div>
        {temperatura != null && (
          <div style={{ fontSize: vh(temRecado ? 44 : 64), fontWeight: 800, color: "#bbb" }}>{Math.round(temperatura)}°C</div>
        )}
      </div>
      {/* data */}
      <div style={{ marginTop: temRecado ? 6 : 14, fontSize: vh(temRecado ? 30 : 42), fontWeight: 800, letterSpacing: "0.12em", color: "#C78340" }}>{DIAS[p.dow]}</div>
      <div style={{ marginTop: 4, fontSize: vh(temRecado ? 26 : 34), fontWeight: 600, color: "#aaa" }}>
      ...
      {aniversariantes.length > 0 && (
        <div style={{ width: "100%", maxWidth: 1400, marginTop: 24 }}>
          <div style={{ fontSize: vh(22), fontWeight: 900, letterSpacing: "0.16em", color: "#777" }}>
            🎂 ANIVERSARIANTES DE {MESES[p.mes - 1].toUpperCase()}
          </div>
          {niverHoje.length > 0 && (
            <div style={{ marginTop: 10, background: "#C78340", borderRadius: 16, padding: "14px 24px", fontSize: vh(40), fontWeight: 900, color: "#fff" }}>
              🎉 HOJE: {niverHoje.map((a) => a.nome).join(", ")} — parabéns!
            </div>
          )}
          <div style={{ marginTop: 10, display: "flex", flexWrap: "wrap", gap: 10 }}>
            {aniversariantes.filter((a) => !a.hoje).map((a) => (
              <div key={a.nome + a.dia} style={{ background: "#1c1c1c", border: "2px solid #3a2410", borderRadius: 12, padding: "8px 16px", fontSize: vh(26), fontWeight: 700, color: "#eee" }}>
      ...
      {recados.length > 0 && (
        <div style={{ width: "100%", maxWidth: 1400, marginTop: 24, display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ fontSize: vh(22), fontWeight: 900, letterSpacing: "0.16em", color: "#777" }}>RECADOS</div>
          {recados.slice(0, 5).map((r) => (
            <div key={r.id} style={{ background: "#3a2410", borderLeft: "14px solid #C78340", borderRadius: 16, padding: "18px 24px", fontSize: vh(recados.length > 3 ? 30 : 38), lineHeight: 1.2, fontWeight: 800, color: "#fff", overflowWrap: "anywhere" }}>
              {r.texto}
            </div>
          ))}
        </div>
