...
function desenhar(doc: PDFKit.PDFDocument, d: NfceCupom, W: number, qrPng: Buffer | null) {
  const pad = 7;
  const cw = W - pad * 2;
  doc.fillColor("#000");
  let y = 8;
  ...
  const esq = (txt: string, size = 7.5, bold = false, gap = 2) => {
    if (!txt) return;
  ...
  const par = (a: string, b: string, size = 8, bold = false) => {
  ...
  // ---------- emitente ----------
  centro(d.fantasia || d.emitente, 10, true, 1);
  if (d.fantasia && d.emitente && d.fantasia !== d.emitente) centro(d.emitente, 7, false, 1);
  centro(`CNPJ ${cnpjBonito(d.cnpj)}${d.ie ? ` · IE ${d.ie}` : ""}`, 7, false, 1);
  centro(d.endereco, 7, false, 1);
  centro(d.cidade, 7, false, 3);
  linha();
  centro("DANFE NFC-e", 9, true, 1);
  centro("Documento Auxiliar da Nota Fiscal de Consumidor Eletrônica", 6.5, false, 3);
  if (d.homologacao) centro("*** SEM VALOR FISCAL — HOMOLOGAÇÃO ***", 8, true, 3);
  if (d.contingencia) centro("*** EMITIDA EM CONTINGÊNCIA ***", 8, true, 3);
  linha();
  ...
  for (const it of d.itens) {
    ...
    y += h + 1;
    doc.fontSize(7.5);
    doc.text(`${qtd(it.qtd)} ${it.un} x ${brl(n2(it.unit))}`, pad + cw * 0.3, y, { width: cw * 0.45, align: "right" });
    doc.font("Helvetica-Bold").text(brl(n2(it.total)), pad + cw * 0.75, y, { width: cw * 0.25, align: "right" });
    y += 11;
  }
  linha();

  // ---------- totais ----------
  par("Qtd. total de itens", String(d.itens.length));
  par("Valor total", `R$ ${brl(d.totalProdutos)}`);
  if (d.desconto > 0.004) par("Desconto", `- R$ ${brl(d.desconto)}`);
  par("VALOR A PAGAR", `R$ ${brl(d.totalNota)}`, 11, true);
  y += 1;
  for (const p of d.pagamentos) par(p.forma, `R$ ${brl(p.valor)}`);
  if (d.troco > 0.004) par("Troco", `R$ ${brl(d.troco)}`);
  if (d.tributos > 0.004) {
    esq(`Tributos aprox. R$ ${brl(d.tributos)} — Lei 12.741/2012`, 6.5, false, 2);
  }
  linha();

  // ---------- consulta / chave ----------
  centro("Consulte pela Chave de Acesso em", 7, false, 1);
  centro(d.urlChave || "www.sefaz.rs.gov.br/nfce/consulta", 7, true, 3);
  centro(chaveBonita(d.chave), 7.5, false, 4);

  // ---------- consumidor ----------
  if (d.cpfDest) centro(`CONSUMIDOR — CPF ${cpfBonito(d.cpfDest)}`, 7.5, true, 1);
  else if (d.cnpjDest) centro(`CONSUMIDOR — CNPJ ${cnpjBonito(d.cnpjDest)}`, 7.5, true, 1);
  else centro("CONSUMIDOR NÃO IDENTIFICADO", 7.5, true, 1);
  if (d.nomeDest) centro(d.nomeDest, 7, false, 1);
  y += 2;
  centro(`NFC-e nº ${d.numero}  Série ${d.serie}  ${dataBonita(d.emissao)}`, 7, false, 3);
  if (d.protocolo) centro(`Protocolo de autorização: ${d.protocolo}`, 7, false, 1);
  if (d.protocoloEm) centro(dataBonita(d.protocoloEm), 7, false, 3);
  ...
  centro("Obrigado pela preferência!", 7.5, false, 2);
  return y;
}

// Gera o cupom a partir do XML autorizado da NFC-e.
export async function gerarNfceCupomPdf(xml: string, larguraMm = 80): Promise<Buffer> {
  const d = lerXml(xml);
  ...
  const yFinal = desenhar(medidor, d, W, qrPng);
  medidor.end();
