# Monta o pacote do agente de impressão e gera o instalador (AgenteImpressao-Setup.exe).
# Requisitos: Node instalado + Inno Setup 6 instalado.
# Uso:  powershell -ExecutionPolicy Bypass -File build.ps1

$ErrorActionPreference = "Stop"
$root    = Split-Path -Parent $MyInvocation.MyCommand.Path      # ...\installer-impressao
$src     = Join-Path (Split-Path -Parent $root) "agente-impressao"
...
Write-Host "==> Instalando dependencias (pdf-to-printer)..." -ForegroundColor Cyan
Push-Location $app
...
foreach ($f in @("config.json","agente.log","agente.pid","bandeja.pid")) {
...
& $iscc (Join-Path $root "impressao.iss") | Out-Host

$out = Join-Path $root "Output\AgenteImpressao-Setup.exe"
if (Test-Path $out) {
