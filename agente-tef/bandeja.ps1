# Ícone na bandeja (perto do relógio) que roda o agente TEF em segundo
...
$dataDir = if ($env:ProgramData) { Join-Path $env:ProgramData "AgenteTEF" } else { $dir }
...
# senão viram dois agentes brigando pela porta 8544.
...
$icon.Text    = "Agente TEF"
$icon.Visible = $true

$menu = New-Object System.Windows.Forms.ContextMenuStrip
$miLog  = $menu.Items.Add("Ver log")
$miStatus = $menu.Items.Add("Ver status do TEF")
...
$miStatus.add_Click({ Start-Process "http://localhost:8544/status" })
$miRe.add_Click({
  try { if ($global:proc -and -not $global:proc.HasExited) { $global:proc.Kill() } } catch {}
  Start-Agente
  $icon.ShowBalloonTip(1500, "Agente TEF", "Reiniciado.", [System.Windows.Forms.ToolTipIcon]::Info)
})
$miSair.add_Click({
  try { if ($global:proc -and -not $global:proc.HasExited) { $global:proc.Kill() } } catch {}
...
$timer.add_Tick({
  if (-not $global:proc -or $global:proc.HasExited) {
    Start-Agente
    $icon.Text = "Agente TEF: reiniciando..."
  } else {
    $icon.Text = "Agente TEF: rodando"
  }
})
$timer.Start()

$icon.ShowBalloonTip(2000, "Agente TEF", "Rodando em segundo plano.", [System.Windows.Forms.ToolTipIcon]::Info)
