# Monta o pacote do agente TEF e gera o instalador (AgenteTEF-Setup.exe).
...
$src     = Join-Path (Split-Path -Parent $root) "agente-tef"
...
Copy-Item (Join-Path $src "intpos.mjs")   $app
Copy-Item (Join-Path $src "simulador-gp.mjs") $app
...
Write-Host "==> Instalando dependencias (nenhuma alem do Node)..." -ForegroundColor Cyan
Push-Location $app
